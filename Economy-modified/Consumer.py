...
class Consumer(object):
    ...
    def Utility(self,output_list):
        ...
        weighted_utility_subc = np.multiply(np.power(c_list ,y) ,1)
        ...
        for i in range(len(weighted_utility_subc)):
            if (i == 0) or (i % self.g == 0 and i < self.g * self.c):
                utility_subc = np.append(utility_subc,np.power(np.sum(weighted_utility_subc[i:i + self.g]),(1 - s)/y))
        '''
        Calculate the disutility that comes from supplying factors
        '''
        # Define twighted_utility_subf as an array containing the un-summed elements of the factor supply part of the utility function for each consumer
        ...
        while i < self.c:
            j = 0
            while j < self.f:
                tweighted_utility_subf = np.append(tweighted_utility_subf,(np.power(f_list[(i * self.f) + j],1.9) / 1.9))
                j += 1
            i += 1
        ...
        for i in range(len(tweighted_utility_subf)):
            if i == 0 or (i % self.f == 0 and i < self.c * self.f):
                weighted_utility_subf = np.append(weighted_utility_subf,np.multiply((tweighted_utility_subf[i:i + self.f]),self.B[i:i+self.f]))
        ...
        '''       
        Combine consumption and factor supply parts of the utility function
        '''
        ...
        utility_list = utility_subc - utility_subf
        return utility_list